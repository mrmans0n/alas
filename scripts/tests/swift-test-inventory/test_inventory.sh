...
SWIFT
cat > "${sandbox}/AlasTests/IndirectEnumSuite.swift" <<'SWIFT'
import Testing

@Suite indirect enum IndirectTests {
...
SWIFT
cat > "${sandbox}/AlasTests/EscapedIdentifierSuite.swift" <<'SWIFT'
import Testing
...
grep -qx -- '-only-testing AlasTests/IndirectTests' <<<"${selectors}"
