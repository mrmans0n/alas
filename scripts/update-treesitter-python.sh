#!/usr/bin/env bash
# Re-vendors ThirdParty/TreeSitterPython from tree-sitter/tree-sitter-python
...
DEST="$ROOT/ThirdParty/TreeSitterPython"
...
mkdir -p \
  ...
  "$DEST/bindings/swift/TreeSitterPython" \
  "$DEST/queries"
...
install -m 0644 "$SRC/bindings/swift/TreeSitterPython/python.h" "$DEST/bindings/swift/TreeSitterPython/python.h"
install -m 0644 "$SRC/queries/highlights.scm"                   "$DEST/queries/highlights.scm"
install -m 0644 "$SRC/LICENSE"                                  "$DEST/LICENSE"

REV="$(git -C "$SRC" rev-parse HEAD)"
echo "✓ vendored $TAG ($REV) into ThirdParty/TreeSitterPython"
