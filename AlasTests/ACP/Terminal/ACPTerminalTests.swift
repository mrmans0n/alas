...
struct ACPTerminalTests {
    ...
    func descendantCleanupRunsOffMainActor() throws {
        ...
        #expect(source.contains("var queue: [DescendantKey] = [root]"))
        #expect(source.contains("processStartTime(of: parent.pid) == parent.startedAt"))
        #expect(source.contains("guard let identity = processIdentity(of: child),\n                      identity.parentPid == parent.pid"))
        #expect(source.contains("parentPid: pid_t(info.pbi_ppid)"))
        #expect(source.contains(#""pid=,pgid=""#))
        #expect(source.contains("nonisolated private static func signalTargets"))
        #expect(source.contains("nonisolated static func currentlyMatching(_ keys: Set<DescendantKey>) -> Set<DescendantKey>"))
