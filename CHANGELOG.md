...
- Refuse scheduled worktree cleanup when initialized submodule history is not remote-reachable, bind submodule remote-ref state to deletion authorization, finalize reports promptly after schedule cancellation, retain worktrees when report finalization fails, and reject completions whose serialized request exceeds the socket payload limit (#1465).
