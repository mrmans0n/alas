...
- Refuse scheduled worktree cleanup when initialized submodule history is not remote-reachable, bind worktree lineage and superproject/submodule remote refs to deletion authorization, fail settlement when a queued prompt is removed before dispatch, finalize reports promptly after schedule cancellation, retain worktrees when report finalization fails, and reject completions whose serialized request exceeds the socket payload limit (#1465).
